...
process_general_object <- function(xml_query, xml_node, xml_input, from_name) {
  ...
  #if(length(xml_node )==0) {return(xml_query)}
  ...
  } else if (node_type == "Lookup Procedure") {               # Filter
    xml_query <- process_lookup_procdure(xml_query, xml_node, from_name)  
  ...
  #   # print("**************************************")
  #   # print(getNodeSet(xml_query, "//SELECT/TABLE"))
  #   # if( length(getNodeSet(xml_query, "//SELECT"))>0){
  ...
  #   print(outgoing_connectors)
  #   for (i in  1:nrow(outgoing_connectors)){
  #     print("++++++++++++++++++++++++++++")
  #     target_xpath <- paste0("//*[not(name()='INSTANCE') and @NAME='", outgoing_connectors$to_instance[i], "']")
  #     target_node <- xml_find_all(xml_input, target_xpath)
  #     dependent_not_processed <- subset(connectors_unique_df, to_instance == node_name & processed==0)
  #     print(length(dependent_not_processed$processed))
  #     if(length(dependent_not_processed$processed) >= 1){
  #       next
  #     }
...
process_source_qualifier <- function(xml_query, xml_node, from_name) {
 ...
 # create main SELECT node
  ...
  # print(paste0("====== source filter check ",length(condition_node),"==="))
...
process_expression <- function(xml_query, xml_node, from_name) {
  ...
  # print(paste0("//CONNECTOR[@TOINSTANCE='", from_name, "']"))
  # connectors <- xml_find_all(xml_node, paste0("//CONNECTOR[@TOINSTANCE='", from_name, "']")) 
  # for (i in 1:length(connectors)) {
  #   conn <- connectors[i]
  #   at_tofield <- xml_attr(conn, "TOFIELD")
  #   at_fromfield <- xml_attr(conn, "FROMFIELD")
  #   print(paste0(at_fromfield," -> ", at_tofield))
    

  #   # test <- getNodeSet(xml_query, paste0("//SELECT/COLUMN[@alias='", at_fromfield, "']"))
  #   # xml_remove(test[[1]])

  #   current_column_node <- getNodeSet(xml_query, paste0("//SELECT/COLUMN[@alias='", at_fromfield, "']"))[[1]]
  #   #xml_remove(current_column_node, free=TRUE)
  #   # xpathApply(xml_query, paste0("//COLUMN[@alias='", at_fromfield, "']"), 
  #   #              addAttributes,alias = at_tofield, name="ssss")
    
  #   # at <- xmlAttrs(current_column_node)
  #   # at['alias'] <- at_tofield
  #   # #removeNodes(current_column_node,free = rep(FALSE, length(current_column_node)))
  #   # print(xml_query)
  #   at <- xmlAttrs(current_column_node)

  #   ### TODO: how to dedup?
  #   if(at['alias'] == at_tofield){
  #     next
  #   }
  #   at['alias'] <- at_tofield
  #   newXMLNode("COLUMN", attrs = at, parent = select_node)
